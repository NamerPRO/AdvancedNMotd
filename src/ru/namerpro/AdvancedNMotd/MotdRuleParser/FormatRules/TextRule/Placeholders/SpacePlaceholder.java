...
public class SpacePlaceholder implements IPlaceholder {
    ...
    public Pair<String, Integer> apply(String lineToParse, Limits limits, PlaceholderArguments arguments, boolean isScreeningSmart) throws RuleException {
        if (arguments.getCount() > 1) {
            throw new RuleException("Placeholder \"%s: <numberOfSpaces>%\" must have one or zero arguments, but presented in string as \"" + lineToParse.substring(limits.start, limits.end + 1) + "\". Line: \"" + lineToParse + "\". Index: " + limits.start + ".");
        }
        StringBuilder spacesToInsert = new StringBuilder(arguments.getCount() == 0 ? " " : "");
        if (arguments.getCount() != 0) {
            int amountOfSpaces;
            try {
                amountOfSpaces = Integer.parseInt(arguments.getArgument(0));
            } catch (NumberFormatException ignored) {
                throw new RuleException("Argument of placeholder \"%s: <numberOfSpaces>%\" must be a positive number of integer type! Line: \"" + lineToParse + "\". Index: " + limits.start + ".");
            }
            if (amountOfSpaces <= 0) {
                throw new RuleException("Argument of placeholder \"%s: <numberOfSpaces>%\" must be a positive number of integer type! Line: \"" + lineToParse + "\". Index: " + limits.start + ".");
            }
            for (int i = 0; i < amountOfSpaces; ++i) {
                spacesToInsert.append(' ');
            }
        }
        String parsedLine = lineToParse.substring(0, limits.start) + spacesToInsert + lineToParse.substring(limits.end + 1);
