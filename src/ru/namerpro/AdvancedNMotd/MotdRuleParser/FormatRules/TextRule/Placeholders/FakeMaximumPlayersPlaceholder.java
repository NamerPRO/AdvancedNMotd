...
public class FakeMaximumPlayersPlaceholder implements IPlaceholder {
    ...
    public Pair<String, Integer> apply(String lineToParse, Limits limits, PlaceholderArguments arguments, boolean isScreeningSmart) throws RuleException {
        if (arguments.getCount() != 0) {
            throw new RuleException("Placeholder \"%fakeMaxPlayers%\" cannot have arguments, but presented in line as \"" + lineToParse.substring(limits.start, limits.end + 1) + "\". Line: \"" + lineToParse + "\". Index: " + limits.start + ".");
        }
        if(!ConfigurationManager.config.getBoolean("AdvancedNMotd.Configuration.Data")) {
            UniversalActions.universalInstance.sendConsoleMessage(ChatColor.RED + "AdvancedNMotd: Detected \"%fakeMaxPlayers%\" placeholder, but it was disabled in configuration! To enable it set \"AdvancedNMotd.Configuration.Data\" field to true in \"config.yml\". To dismiss this text and get the same result, screen both '%' symbols by placing '|' before each like this: \"|%\". Line: \"" + lineToParse + "\". Index: " + limits.start + ".");
            return new Pair<>(lineToParse, limits.end);
        }
        String parsedLine = lineToParse.substring(0, limits.start) + ConfigurationManager.data.getInt("Data.FakeMaxPlayers") + lineToParse.substring(limits.end + 1);
