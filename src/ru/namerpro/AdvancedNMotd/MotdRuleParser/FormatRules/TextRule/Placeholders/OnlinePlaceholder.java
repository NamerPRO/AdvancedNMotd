...
public class OnlinePlaceholder implements IPlaceholder {
    ...
    public Pair<String, Integer> apply(String lineToParse, Limits limits, PlaceholderArguments arguments, boolean isScreeningSmart) throws RuleException {
        if (arguments.getCount() != 0) {
            throw new RuleException("Placeholder \"%online%\" cannot have arguments, but presented in line as \"" + lineToParse.substring(limits.start, limits.end + 1) + "\". Line: \"" + lineToParse + "\". Index: " + limits.start + ".");
        }
        String parsedLine = lineToParse.substring(0, limits.start) + UniversalActions.universalInstance.getOnlinePlayers() + lineToParse.substring(limits.end + 1);
